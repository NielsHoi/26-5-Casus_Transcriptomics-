...
## Inleiding
Reumatoïde artritis (RA) is een chronische auto-immuunziekte waarbij het immuunsysteem het synovium, ook wel het gewrichtsslijmvlies genoemd, aanvalt. Hierdoor kunnen ontsteking van het gewrichtsslijmvlies, afbraak van kraakbeen en vorming van abnormaal weefsel (pannus) ontstaan [[1]](https://pmc.ncbi.nlm.nih.gov/articles/PMC8780115/pdf/ijms-23-00905.pdf). Als gevolg hiervan kunnen patiënten klachten ontwikkelen zoals pijn, zwelling en stijfheid van de gewrichten. Factoren die bijdragen aan het ontstaan van RA zijn onder andere erfelijke aanleg, omgevingsfactoren zoals roken en ontregeling van het immuunsysteem. Hoewel verschillende behandelingen beschikbaar zijn, reageert niet iedere patiënt voldoende op de huidige therapieën. Daarom blijft onderzoek naar de moleculaire mechanismen achter RA belangrijk voor de ontwikkeling van effectievere behandelmethoden [[2]](https://core.ac.uk/reader/77600864?utm_source=linkout).

Transcriptomics is een techniek waarmee verschillen in genexpressie tussen groepen kunnen worden onderzocht. Door de genexpressie van patiënten met RA te vergelijken met die van gezonde controles kunnen genen worden geïdentificeerd die mogelijk betrokken zijn bij ziekte-gerelateerde processen. Analyse van deze genen kan bijdragen aan een beter begrip van de biologische mechanismen en signaalroutes die een rol spelen bij de pathogenese van reumatoïde artritis [[3]](https://pmc.ncbi.nlm.nih.gov/articles/PMC10603609/pdf/ac3c02029.pdf).
### Onderzoeksdoel
Het doel van dit onderzoek is om te bepalen welke genen significant verschillend tot expressie komen tussen patiënten met reumatoïde artritis en gezonde controles. Daarnaast wordt onderzocht welke biologische processen en signaalroutes geassocieerd zijn met deze differentieel geëxpresseerde genen met behulp van Gene Ontology- en KEGG-pathway-analyses. Ten slotte worden de gevonden verschillen in genexpressie geprojecteerd op de [Rheumatoid Arthritis Pathway](https://www.kegg.jp/entry/map05323) om inzicht te krijgen op het effect van de veranderde genexpressie op mensen met RA.

## Materiaal en Methode
Om te onderzoeken of er verschillen in genexpressie tussen mensen met RA en gezonde mensen werd data in de vorm van FastQ-bestanden gebruikt van 4 gezonde mensen, en 4 mensen met Reumatoïde Artritis. De data is afkomstig vanuit het synoviumbiopt: weefsel afkomstig uit het gewrichtsslijmvlies. Personen met RA zijn positief getest op ACPA, personen zonder RA negatief. Van de zieke mensen is het minimaal twaalf maanden bekend dat de mensen ziek zien met RA. De gebruikte data is te vinden onder ["Data>RawData"](https://github.com/NielsHoi/26-5-Casus_Transcriptomics-/tree/main/Data/RawData). 

In RStudio (versie 2026.01.0+392) werd met het pakket Rsubread (Versie 2.24.0)[[4]](https://bioconductor.org/packages//release/bioc/html/Rsubread.html) de sequencingreads uitgelijnd tegen het humane [referentiegenoom GRCh38.p14](https://www.ncbi.nlm.nih.gov/datasets/genome/GCF_000001405.40/). Voor iedere sample werd een [BAM-file](https://github.com/NielsHoi/26-5-Casus_Transcriptomics-/tree/main/Data/ProcessedData) gegenereerd. Vervolgens werd met de functie featureCounts reads toegewezen aan genen op basis van de GTF-annotatie van [referentiegenoom GRCh38.p14](https://www.ncbi.nlm.nih.gov/datasets/genome/GCF_000001405.40/). Het resultaat was een [count matrix](https://github.com/NielsHoi/26-5-Casus_Transcriptomics-/tree/main/Data/ProcessedData) waarin voor ieder gen het aantal gemapte reads per sample werd weergegeven. 

Nadat de count matrix gemaakt was, kon er een differrentiele genexpressie-analyse gedaan worden met behulp van pakket DESeq2 (Versie 1.50.2) [[5]](https://bioconductor.org/packages//release/bioc/html/DESeq2.html) waarbij de genexpressie van de mensen met RA vergeleken kan worden met de gezonde mensen, hierbij worden genen met een P-waarde lager dan 0,05 beschouwd als statistisch significant verschillend. De genexpressie van verschillende genen kan dan worden uitgebeeld in een volcanoplot met behulp van de functie EnhancedVolcano (Versie 1.28.2) [[6]](https://bioconductor.org/packages//release/bioc/html/EnhancedVolcano.html).

De gevonden significant differentieel geëxpresseerde genen werden gebruikt voor een Gene Ontology-verrijkingsanalyse met het pakket clusterProfiler (versie 4.18.4) [[7](https://bioconductor.org/packages//release/bioc/html/clusterProfiler.html). GO-termen met een aangepaste p-waarde kleiner dan 0,05 werden als significant beschouwd. Vervolgens werd een KEGG pathway-verrijkingsanalyse uitgevoerd met behulp van het pakket clusterProfiler. Menselijke genen werden geconverteerd naar Entrez-ID's met behulp van het pakket org.Hs.eg.db (versie 3.22.0) [[8]](https://bioconductor.org/packages//release/data/annotation/html/org.Hs.eg.db.html). Significante pathways werden geïdentificeerd met een aangepaste p-waarde kleiner dan 0,05. De expressiewaarden werden gevisualiseerd in de KEGG rheumatoid arthritis pathway met behulp van het pakket pathview (versie 1.50.0)[[9]](https://www.bioconductor.org/packages//release/bioc/html/pathview.html). Zowel het volledige script als delen van het script voor specifieke stappen wat gebruikt werd in Rstudio is te vinden bij ["Script"](https://github.com/NielsHoi/26-5-Casus_Transcriptomics-/tree/main/Script). 
...
[[3]](https://pmc.ncbi.nlm.nih.gov/articles/PMC10603609/pdf/ac3c02029.pdf) Robles-Remacho, A., Sanchez-Martin, R. M., & Diaz-Mochon, J. J. (2023). Spatial Transcriptomics: Emerging Technologies in Tissue Gene Expression Profiling. In Analytical Chemistry (Vol. 95, Number 42, pp. 15450–15460). American Chemical Society. https://doi.org/10.1021/acs.analchem.3c02029

[[4]](https://bioconductor.org/packages//release/bioc/html/Rsubread.html) https://bioconductor.org/packages//release/bioc/html/Rsubread.html

[[5]](https://bioconductor.org/packages//release/bioc/html/DESeq2.html) https://bioconductor.org/packages//release/bioc/html/DESeq2.html

[[6]](https://bioconductor.org/packages//release/bioc/html/EnhancedVolcano.html) https://bioconductor.org/packages//release/bioc/html/EnhancedVolcano.html

[[7]](https://bioconductor.org/packages//release/data/annotation/html/clusterProfiler.html) https://bioconductor.org/packages//release/data/annotation/clusterProfiler.html

[[8]](https://bioconductor.org/packages//release/data/annotation/html/org.Hs.eg.db.html) https://bioconductor.org/packages//release/data/annotation/html/org.Hs.eg.db.html

[[9]](https://www.bioconductor.org/packages//release/bioc/html/pathview.html) https://www.bioconductor.org/packages//release/bioc/html/pathview.html

[[10]](https://www.nature.com/articles/s41392-021-00828-5) He, Y., Sun, M. M., Zhang, G. G., Yang, J., Chen, K. S., Xu, W. W., & Li, B. (2021). Targeting PI3K/Akt signal transduction for cancer therapy. In Signal Transduction and Targeted Therapy (Vol. 6, Number 1). Springer Nature. https://doi.org/10.1038/s41392-021-00828-5
